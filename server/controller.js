...
//SELECT * FROM similarsongs OFFSET floor(random() * (SELECT COUNT(*) FROM songinformation)) LIMIT 3;
//
const getArtist = async (req, res, next) => {
  await pool.query(
    "SELECT * FROM songinformation OFFSET floor(random() * 1000000) LIMIT 1;",
    (err, result) => {
      if (err) {
        throw err;
      }
...
const getSimilarSongs = async (req, res, next) => {
  await pool.query(
    "SELECT * FROM similarsongs OFFSET floor(random() * 100) LIMIT 3;",
    (err, result) => {
      if (err) {
        throw err;
      }
